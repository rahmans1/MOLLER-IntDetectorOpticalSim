int extractRunInfo(TString input_folder, TString o_fname){

TString out="QuartzSizeX,QuartzSizeY,QuartzSizeZ,LowerInterfacePlane,UpperInterfacePlane,LowerConeFrontFaceAngle,LowerConeBackFaceAngle,LowerConeSideFaceAngle,QuartzInterfaceOpeningZ,QuartzInterfaceOpeningX,PMTInterfaceOpeningZ,PMTInterfaceOpeningX,QuartzToPMTOffsetInZ,QuartzBevel,QuartzRotation,DetectorRotation,HitRegion,Mean,ExNoise\n";
std::ofstream ofs (o_fname.Data(), std::ofstream::out);
for (int i=1;i<=1936;i++){
 TString fname = Form("%s/MOLLEROptConfig_%04d.root", input_folder.Data(), i);
