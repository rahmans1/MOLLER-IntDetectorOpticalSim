...
LightGuideLowerInterface = list(map(str, generator.integers(65,96,size=1+int(x[index:])-int(x[:index-1]))))
LightGuideUpperInterface = list(map(str, generator.integers(611,622,size=1+int(x[index:])-int(x[:index-1]))))
LightGuidePMTInterfaceOpeningX = list(map(str, generator.integers(70,71, size=1+int(x[index:])-int(x[:index-1]))))
LightGuidePMTInterfaceOpeningZ = list(map(str, generator.integers(70,71, size=1+int(x[index:])-int(x[:index-1]))))
QuartzSizeZ = generator.integers(10,31,size=1+int(x[index:])-int(x[:index-1]))
QuartzSizeX = generator.integers(177,178,size=1+int(x[index:])-int(x[:index-1]))
QuartzSizeY = list(map(str, generator.integers(60,61,size=1+int(x[index:])-int(x[:index-1]))))
...
QuartzBevelSize = list(map(str, 0.4+(0.6-0.4)*generator.random(size=(1+int(x[index:])-int(x[:index-1])))))
QuartzRotX = list(map(str, generator.integers(-3,-2,size=(1+int(x[index:])-int(x[:index-1])))))
PolarRotation = list(map(str, generator.integers(3,4,size=(1+int(x[index:])-int(x[:index-1])))))
LightGuideQuartzToPMTOffset = list(map(str, generator.integers(-10,11,size=(1+int(x[index:])-int(x[:index-1])))))
