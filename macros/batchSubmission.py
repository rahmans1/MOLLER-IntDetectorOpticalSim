...
LightGuideLowerInterface = list(map(str, np.random.uniform(65,95,size=1+int(x[index:])-int(x[:index-1]))))
...
LightGuideQuartzInterfaceOpeningX = np.array(float(QuartzSizeX)) + 8
LightGuideQuartzInterfaceOpeningZ = np.array(float(QuartzSizeZ)) + 7
...
LightGuideQuartzToPMTOffset = list(map(str, np.random.uniform(-10,10,size=(1+int(x[index:])-int(x[:index-1])))))
