...
Int_t GetGraph(Int_t fA, Int_t oF, Int_t hR);
Int_t Get2DHist(Int_t oF, Int_t hR, TString type);
...
std::vector <TH2D*> fAbA_Exnse;
std::vector <TH1D*> FitHist;
std::vector <TF1*> FitFunc;
...
void ExtractResults(TString *dir = 0)
{
  ...
  fAbins = fAmax - fAmin +1;
  bAbins = bAmax - bAmin +1;
  oFbins = (oFmax - oFmin)/2 +1;
  ...
  while(std::getline(rfiles, line)){
    ...
    cout << endl  << endl;
    cout << "Sim " << cnt << " File " << line.data() << endl  << endl  << endl;
    ...
    of = (TVectorD*)file->Get("QuartzToPMTOffsetInZ");
    ...
    hst = (TH1D*)tmp->Clone(Form("CEH_%d",cnt));//"CEH_%s",runID.Data()));
    hst->SetTitle(Form("Photoelectron Distribution %d",cnt));
    hst->GetXaxis()->SetTitle("Photoelectrons");
    hst->SetDirectory(0);   

    m = GetGraph(fAi,oFi,hRi);
    ...
    if(hRi == 1){
      ...
      if(m >= 0){
	fA_PEmean[m]->SetPoint(fA_PEmean[m]->GetN(),bAi,fitP[1]);
	fA_PEmean[m]->SetPointError(fA_PEmean[m]->GetN()-1,0,fitE[1]);
	fA_Exnse[m]->SetPoint(fA_Exnse[m]->GetN(),bAi,pow(fitP[3]/fitP[1],2));
	fA_Exnse[m]->SetPointError(fA_Exnse[m]->GetN()-1,0,2*pow(fitP[3]/fitP[1],2)*sqrt(fitE[3]*fitE[3]/fitP[3]/fitP[3] + fitE[1]*fitE[1]/fitP[1]/fitP[1]));
      ...
      n = Get2DHist(oFi,hRi,"mean");
      if(n >= 0) fAbA_PEmean[n]->SetBinContent(fAbA_PEmean[n]->FindBin(fAi,bAi),fitP[1]);

      n = Get2DHist(oFi,hRi,"exnse");
      if(n >= 0) fAbA_Exnse[n]->SetBinContent(fAbA_Exnse[n]->FindBin(fAi,bAi),pow(fitP[3]/fitP[1],2));
      
    }
    else if(hRi == 2){
      
      if(m >= 0){
      	fA_PEmean_hR2[m]->SetPoint(fA_PEmean_hR2[m]->GetN(),bAi,hst->GetMean());
      	fA_PEmean_hR2[m]->SetPointError(fA_PEmean_hR2[m]->GetN()-1,0,hst->GetMeanError());	
      }

      n = Get2DHist(oFi,hRi,"mean");
      if(n >= 0) fAbA_PEmean[n]->SetBinContent(fAbA_PEmean[n]->FindBin(fAi,bAi),hst->GetMean());

    }    
    else if(hRi == 3){
	
      if(m >= 0){
      	fA_PEmean_hR3[m]->SetPoint(fA_PEmean_hR3[m]->GetN(),bAi,hst->GetMean());
  ...
  for(int f = 0; f < fAbA_Exnse.size(); f++){
    fAbA_Exnse[f]->Write();
  }
...
Int_t Get2DHist(Int_t oF, Int_t hR, TString type)
{
  ...
  if(type == "mean"){
    ...
    for(int k = 0; k < fAbA_PEmean.size(); k++){
      if(fAbA_PEmean[k]){
	name = fAbA_PEmean[k]->GetName();
	if(name.Contains(Form("oF%d_hR%d",oF,hR))){
	  return k;
	}
      }
    }
    TH2D* hst = new TH2D(Form("PEMean_oF%d_hR%d",oF,hR),Form("PEMean_oF%d_hR%d",oF,hR),fAbins,fAmin-0.5,fAmax+0.5,bAbins,bAmin-0.5,bAmax+0.5);
    hst->SetDirectory(0);
    hst->GetXaxis()->SetTitle("Lower Funnel Upstream Angle [Deg]");
    hst->GetYaxis()->SetTitle("Lower Funnel Downstream Angle [Deg]");
    fAbA_PEmean.push_back(hst);
    return fAbA_PEmean.size()-1;
  }
  if(type == "exnse"){
  
 
    for(int k = 0; k < fAbA_Exnse.size(); k++){
      if(fAbA_Exnse[k]){
	name = fAbA_Exnse[k]->GetName();
	if(name.Contains(Form("oF%d_hR%d",oF,hR))){
	  return k;
	}
      }
    }
    TH2D* hst = new TH2D(Form("Exnse_oF%d_hR%d",oF,hR),Form("Exnse_oF%d_hR%d",oF,hR),fAbins,fAmin-0.5,fAmax+0.5,bAbins,bAmin-0.5,bAmax+0.5);
    hst->SetDirectory(0);
    hst->GetXaxis()->SetTitle("Lower Funnel Upstream Angle [Deg]");
    hst->GetYaxis()->SetTitle("Lower Funnel Downstream Angle [Deg]");
    fAbA_Exnse.push_back(hst);
    return fAbA_Exnse.size()-1;
  }
...
Int_t GetGraph(Int_t fAng, Int_t oF, Int_t hR)
{
  TString name;
  ...
  if(hR == 1){
	...
	if(name.Contains(Form("fA%d_oF%d_hR%d",fAng,oF,hR))){
	  return k;
	}
      }
    }
    ...
    gr->SetName(Form("PEMean_fA%d_oF%d_hR%d",fAng,oF,hR));
    gr2->SetName(Form("Exnse_fA%d_oF%d_hR%d",fAng,oF,hR));
...
void DoFit(TH1D *hst, Double_t *fitR, Double_t *fitE)
{
  ...
  Int_t nfound2 = s->Search(hst,5,"goff",0.01);
  ...
  //hst->Draw();
  //fitsnr->Draw("lsame");
  FitHist.push_back(hst);
  FitFunc.push_back(fitsnr);
